...
BUILD = 20070724RZ
VERSION = 4.1.1
PROGRAM = Twinscan/N-SCAN
PAIRAGON_VERSION = 1.01
...
install:
	...
	for f in $(ISCAN_SCRIPTS); do \
		install -m 0755 $$f $(DESTDIR)/bin/$$(basename $$f .pl) ;\
	done
	mkdir -p $(DESTDIR)/share/iscan
	cp -r $(DATA) $(DESTDIR)/share/iscan
	mkdir -p $(DESTDIR)/lib/perl5
	install -m 0644 $(PERL_LIBS) $(DESTDIR)/lib/perl5

install-pairagon: install
	install -m 0755 $(PAIRAGON_EXECUTABLES) $(DESTDIR)/bin
