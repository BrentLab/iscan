...
int main (int argc, char *argv[]) {
	...
	zOutputSFVec(&trellis,sfv,NULL,NULL);
	...
	if(snp_mode){
		...
		while(ad != NULL){
			if(!(ad->score_diff < 0.01 && ad->score_diff > -0.01)){
				zOutputSFVec(&trellis,ad->sfv,NULL,ad->header);
			}
			zFreeAlleleDecode(ad);
			zFree(ad);
			ad = zPtrListMoveNext(ptrl);
...
void zOutputSFVec(zTrellis* trellis, zSFVec* sfv, char* filename, char* header){
  ...
  char *def;
    ...
    def = trellis->dna->seqname;
    if(filename)
      def = filename;
		zInitGTFVec(&gtfvec, sfv->size);
		zSFVec2GTFVec(trellis->hmm, sfv, &gtfvec, def);
		zSortGTFVec(&gtfvec);
...
void zWriteHeaders(zTrellis* trellis, char* full_command_line,char* parameter_file_name, 
	...
	if(snp_file != NULL){
 	...
 	/* fprintf(outfile, "# Completed in %ld CPU seconds\n",clock()/CLOCKS_PER_SEC); */
	/* EVAN - this method of getting the time does not suffer from the overflow problems
	   that the above method does during long runs */
